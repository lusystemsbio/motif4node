...
enrichment_single <- function(all.circuits, all.scores, motif_list, new_ind, decreasing = decreasing, topCircuits  = 600, nhill = 20) {
...
####        topCircuits -- the number of top circuits the enrichment analysis emphasizing on
####        nhill -- Hill coefficient for the Hill function as the weighting factor for motif coutns, large n makes the Hill function more binary
enrichment_coupling <- function(all.circuits, all.scores, motif_list, new_ind, overlap_list, decreasing = decreasing, if_overlap = 2, 
...
enrichment_coupling_all_cases <- function(all.circuits, all.scores, motif_list, new_ind, overlap_list, decreasing = decreasing, 
                                          topCircuits  = 600, nhill = 20) {
...
my_analysis <- function(all.circuits, all.scores, ylim = NULL, color_breaks = NULL, filename = NULL, decreasing){
  motif_list = generate_motif_list()
  new_ind = generate_index_conversion()
  overlap_list = generate_overlap_data()
  
  outcome_single = enrichment_single(all.circuits, all.scores, motif_list, new_ind, decreasing = decreasing, topCircuit = 600) 
  ...
  outcome_coupling_all = enrichment_coupling_all_cases(all.circuits, all.scores, motif_list, new_ind, overlap_list, 
                                                       decreasing = decreasing, topCircuit = 600)
...
#### term2 enrichment #### 
load("/work/lulab/Ben/distmat/NEWMAN_SUBSAMPLING/inception/split/Iterative_Subsampling/ASSOCIATION/experimental/sc_data_ben/hgForebrain/distances.scaled.2node")
load("/work/lulab/Ben/BenStuff/research/projects/oldhome/scripts/networks/rset/rsetlists/input/ALLUNIQUEMODELS_final.Rdata")
scoremat <- data.frame(score = unlist(distances), index = 1:length(distances))
motif_list = generate_motif_list()
new_ind = generate_index_conversion()

exp_stats_df = as.data.frame(matrix(nrow =39, ncol = 1000))
for(i in 1:1000){
  scoremat$index = sample(scoremat$index)
  exp_stats_df[,i] = enrichment_single(all.circuits= ALLUNIQUEMODELS, all.scores=scoremat, motif_list, new_ind=new_ind, decreasing = F, topCircuits  = 600, nhill = 20)[,1]
}

save(exp_stats_df, file = "exp_stats_REPLACEME")
